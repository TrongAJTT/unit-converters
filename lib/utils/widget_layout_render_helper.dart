...
import 'package:unit_converters/utils/size_utils.dart';
      ...
      builder: (context, constraints) {
        ...
        if (decorator.widthWidget1 != null) {
          ...
          if (dimension1.type == DimensionType.fixed) {
            children.add(SizedBox(width: dimension1.value, child: widget1));
          } else if (dimension1.type == DimensionType.expanded) {
            children.add(Expanded(child: widget1));
          } else {
            // For percentage and flex, calculate actual width
            final calculatedWidth = dimension1.calculate(
              constraints.maxWidth - spacing.horizontalSpacing,
            );
            children.add(SizedBox(width: calculatedWidth, child: widget1));
          }
        } else {
        ...
        if (decorator.widthWidget2 != null) {
          ...
          if (dimension2.type == DimensionType.fixed) {
            children.add(SizedBox(width: dimension2.value, child: widget2));
          } else if (dimension2.type == DimensionType.expanded) {
            children.add(Expanded(child: widget2));
          } else {
            // For percentage and flex, calculate actual width
            final remainingWidth =
                constraints.maxWidth - spacing.horizontalSpacing;
            final calculatedWidth = dimension2.calculate(remainingWidth);
            children.add(SizedBox(width: calculatedWidth, child: widget2));
          }
        } else {
          // If no dimension specified, use as-is
  ...
  static Widget _buildSizedWidget(
    ...
    switch (dimension.type) {
      case DimensionType.fixed:
        return SizedBox(width: dimension.value, child: widget);
      ...
      case DimensionType.flex:
        ...
        return SizedBox(width: calculatedWidth, child: widget);
    }
  }
...
class GridBuilderColumnProfile {
  ...
  static List<GridBuilderColumnProfile> minWidthPerColumn(
    ...
    profiles.add(
      const GridBuilderColumnProfile(widthThreshold: 0, numberOfColumns: 1),
    );
...
class GridBuilderHelper extends StatelessWidget {
        ...
        return GridView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            if (decorator?.childPadding != null) {
              child = Padding(padding: decorator!.childPadding!, child: child);
            }
  ...
  static GridBuilderHelper fixed({
    ...
    return GridBuilderHelper(
      columnProfiles: [
        GridBuilderColumnProfile(widthThreshold: 0, numberOfColumns: columns),
      ],
      builder: builder,
      itemCount: itemCount,
      decorator: decorator,
