...
import 'package:unit_converters/utils/icon_mapper.dart';
...
class MarkdownInfoParser {
  InfoPage parse(String content) {
    ...
    final title = lines.first.startsWith('# ')
        ? lines.first.substring(2).trim()
        : '';
  ...
  InfoSection _parseSection(List<String> lines) {
    ...
    for (final line in lines.skip(1)) {
      ...
      featureMatch = RegExp(
        r'-\s*\[shape:(circle|square),([a-zA-Z]+)\]\s*([^:]+):\s*(.+)',
      ).firstMatch(line);
      if (featureMatch != null) {
        items.add(
          ColoredShapeInfoItem(
